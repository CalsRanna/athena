              ...
              child: Column(
                ...
                children: [
                  if (models.isNotEmpty)
                    _ModelSwitcher(current: 1, models: models),
                  Expanded(
                    child: chat == null
                        ? Center(
                            child: Text(
    ...
    try {
      ...
      stream.listen(
        (token) {
          setState(() {
            chat!.title =
                '${chat!.title ?? ''}$token'.trim().replaceAll('。', '');
          });
        },
        onDone: () {
          storeChat();
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              SizedBox(width: 12),
              Text(title, style: titleMedium?.copyWith(color: onPrimary)),
              Spacer(),
              Text(
                updatedAt,
...
class _ModelSwitcher extends StatefulWidget {
  const _ModelSwitcher({super.key, this.current = 0, required this.models});
