...
### Chat Preview Card（会话悬浮预览卡，本仓新增）

鼠标停在侧栏会话行上 400ms 后，在行右侧弹出的一张只读浮层。它不是 Claude 的
组件（交互取自 Codex 桌面端），规格由本仓自定：

- **两行内容**：第一行会话标题（`body` 13 / w600 / `textPrimary`，单行省略）；
  第二行是**会话开头那一轮**的 agent 回答正文（`caption` 12 / `textSecondary`，
  最多 3 行省略，换行与缩进压平成单段）。取"首条用户消息之后紧跟的那条回答"，
  不是最新消息——最新一条可能只是工具步骤，或还没收尾的占位。
- **尺寸与底色**：宽 248，圆角 `container`(10)，内边距 12/10，底色 `surfaceMobile`
  + `AthenaShadow.overlay`，不描边（与右键菜单同档，不再新增浮层等级）。
- **位置**：左边缘 = 会话行右边缘 + 8（正好压在侧栏分界线上），顶边与行对齐；
  贴到窗口右 / 下沿时回退，四周各留 8。
- **不吃指针**：卡片只读，整张卡 `IgnorePointer`。指针停在行上才是它的常态；
  卡片压在指针旁边时若把 hover 抢走，行的 hover 底色会"熄"掉。
- **触发与收起**：hover 延迟 400ms（鼠标扫过列表不闪卡）；`onExit`、点选、
  右键立刻收卡。卡片按行 State 归属，某一行销毁时只关自己那张。
- **进出场**：淡入 + 从下浮起约 6px + 从 0.98 放大到 1，进场 140ms（`easeOut`）、
  退场 100ms（`easeIn`）——指针已移开，收卡要跟手。换一条行弹卡时上一张直接
  摘掉（不等淡出），不叠卡。这是全站第一处给浮层加动画的地方：右键菜单与
  对话框仍是瞬时出现，不要顺手去改它们。
- **取数**：只读会话文件**头部**若干行（`loadLeadingMessageRows`，不整文件读），
  结果按 chatId 缓存在 `ChatViewModel`；空结果不缓存，首轮收尾后再悬浮即读到。
...
- **条数 = 整段会话的 user 消息数，与窗口无关**：消息列表的窗口**够小的会话就是
  ...
  **长度与颜色与窗口里的条同一套公式**（`TurnIndicator.barWidthFor` /
  ...
  不弹卡。渲染上分两条路径、逻辑只有一份（`_BarField`）：窗口那几轮各自是控件
  （hover 命中区、弹卡、卡片锚点的 RenderBox），历史那几段用一次 `CustomPaint`
  画完，不为每条建控件（条数可能上百上千）；整列的长度与颜色由一条控制器统一
  补间 120ms。**点未加载的条会先向上翻页把那一轮补进窗口，再滚过去**（最多
  8 页），点了不会没反应。
...
- **位置与形状**：落在消息区左留白内，距留白左缘 12，与正文留 12 的间隙，即条长
  ...
  各留 4 才点得中），**只有这一小条吃指针**，其余地方穿透。**行高随总轮数收缩**
  （`min(12, 可用高 ÷ 总轮数)`，条厚同步变薄）——旧的固定 12 高在几十轮后就会
  `RenderFlex overflow`；很长的会话里条会变密，那时它是位置图而非精确点选。
...
- **颜色**：视口当前所在的那一轮与 hover 那一轮都用 `textRowLabel`（`#52514E`），
  ...
  颜色的过渡 120ms（`easeOut`）。
- **交互**：hover 150ms 后弹出会话预览卡（第一行 = 该轮用户消息，第二行 = 该轮
  回答，用的是同一张 `ChatPreviewCard`）；延迟比侧栏会话行的 400ms 短——条是小
  目标，停上去本身就是明确意图。单击把那一轮滚到视口顶部。
