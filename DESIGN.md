...
`AthenaSettings`，色板在同文件的 `AthenaSettingsColors`。
...
- **左栏 192**：底色 `#FCFCFB` + 右侧 1px `#E4E4E3`。自上而下：**搜索框**
  （高 32、圆角 8、底 `#FEFEFD`、描边 `#E6E6E5`、图标与占位 `#898781`）→
  **分组标题**（12、`#898781`，与行的图标列对齐，上方 28、下方 13）→
  **图标行**（高 32、圆角 8、左内缩 12、图标 16、图标与标签间距 12、标签 14；
  静止字 `#52514F`，选中底 `#E3E3E2` + 近黑字）。
- **右栏内容区**：**纯白**底、左右内边距 24；分区标题 16/600、其下 28。
  行是「标签 + 说明」在左、**控件在右**，上下内边距 16（行高约 69）；标签与说明
  **同号 14**（实测大写高都是 10.0），标签半粗近黑、说明常规 `#898781`。行间是
  1px `#F3F3F3` 发丝线，只跨内容区的左右内边距。
- **分段控件**：轨道 `#F3F3F3`**无描边**、高 32、圆角 8；选中块是**纯白填充 +
  1px `#E7E7E7` 描边**且**铺满轨道高**；选中 12 半粗近黑，未选中 12 常规
  `#898781`。
- **下拉**：白底 + 1px `#E7E7E7` + 高 32 + 圆角 8 + 右端 chevron。
...
- **列表类分区**（Provider / Sentinel / Skills / Experiences）保留「列表 + 详情」
  ...
  靠右侧 1px `#E4E4E3` 与详情分开。

**为什么这套色板单独定义**：Claude 的设置面板用一组**中性灰**（分隔线
`#F3F3F3`、控件描边 `#E7E7E7`、选中底 `#E3E3E2`），而 `AthenaColors` 的对应角色
是**暖灰**（`divider` `#E1E0D9` 等），铺在纯白面板上会偏黄。这与 composer 边框
单独定义是同一条先例：**白底上的中性灰单独定义，不动既有暖灰 token**。深色一套
无法从浅色截图量取，是按同一语义镜像推导的（代码注释里已标注）。
