...
- **字号档位**：Small 0.85 / Medium 1.0 / Large 1.15，由 `AthenaWorkspaceTextSize` 在消息列表内叠加 `TextScaler`
  （`scale = 系统缩放 × 档位系数`），仅覆盖消息与代码，桌面与移动端范围一致。
  composer、空会话 placeholder、轮次导航、审批控件、侧栏、顶栏、设置与弹出菜单保留系统文字缩放，既有几何 token 不随字号档位改变。
