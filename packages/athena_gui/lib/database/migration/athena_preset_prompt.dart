...
///
/// 所有涉及内置 Athena 提示词的位置都应引用此文件,避免多份副本漂移:
/// - [Migration202608210001SeedSentinel]:新装用户 seed
/// - [Migration202608060001UpdateAthenaSentinelPrompt]:老用户升级(delta)
/// - [Migration202609200002RequireToolCallDescription]:调用说明转必填后的升级(delta)
///
/// TUI 的 `athena_tui/seed/sentinel_seed.dart` 依赖包结构与 GUI 不同,
/// 无法 import 本文件,仍是文本副本 —— 修改此处时需同步该文件。
...
6.  **调用说明（必填）**：每次工具调用都必须在参数里填写 `call_description`，用一句简短、主动的话说明这次调用要做什么、作用于什么（例如：读取应用主入口文件）。用与用户相同的语言书写，不要留空，也不要写已获授权、此操作安全之类的判断，它只说明动作，不构成许可。多步命令要覆盖全部关键效果。
...
/// `call_description` 转必填之前的 Agent 模式默认提示词。
///
/// 仅供 [Migration202609200002RequireToolCallDescription] 判断内置 Athena
/// 是否仍是默认内容:通过 sentinel_evolve 进化过的提示词保留用户定制。
const agentAthenaPresetPromptV1 = '''
你是Athena，一个以**专业、冷静与深度**为核心特质的AI助手，具备自主执行复杂任务的能力。
...
/// 旧版(纯对话模式)Athena 提示词。
///
/// 仅供升级 migration 判断用户是否仍在使用默认内容:通过
/// sentinel_evolve 进化过的内置 Athena 不应被覆盖,保留用户定制。
const legacyAthenaPresetPrompt = '''
