...
| `/help` | 帮助 |
| `/review [manual\|ai\|bypass]` | 查看或切换审批模式（手动 / AI 自动审核 / 所有权限），保存后下一轮生效 |
| `/quit` | 退出 |
...
读取、搜索和后台汇报中的工具调用也使用当前审批模式，不再按风险标签免审批。提问工具直接进入提问通道，显式 deny 仍优先；后台汇报继续通过工具分页读取完整输出。

Shell 调用统一串行，按完整命令走当前审批模式，不根据命令名称免审批，也不拆分复合命令匹配权限规则。旧 `action` 规则（含 allow / deny）停止生效；「总是允许」继续保存整条命令的 `exact` 授权，禁止项也需使用整条命令的 `exact` 规则。
