...
- **三层权限模型**：Skill allowed-tools 白名单、用户持久化规则、审批弹窗
...
5. 执行前检查权限（Skill 白名单 → 持久化规则 → 审批弹窗）
...
三层检查（在 `AgentService.run()` 中按顺序）：

1. **Skill allowed-tools**：当前 Skill 上下文中声明了 allowed-tools，命中直接放行；`skill` 工具始终自动放行
2. **用户持久化规则**：`~/.athena/permissions.json` 中的通配符匹配规则
3. **审批弹窗**：以上都不匹配时弹出完整命令预览，用户可选 Allow/Deny + 记忆选项（本次/模式）

SSRF 防护：链路本地地址（169.254.0.0/16）和云元数据地址硬拦截，审批无法覆盖。
...
- `test/agent/` - Agent 层单元测试（工具、权限、Skill、URL 安全）
